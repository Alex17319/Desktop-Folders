...
{		
	...
	[DllImport("user32.dll", SetLastError=true,
			CharSet=CharSet.Unicode, ExactSpelling=true,
			CallingConvention=CallingConvention.Winapi)]
	private static extern bool apiDeleteMenu(IntPtr hMenu, uint uPosition, uint uFlags);
	...
	public bool DeleteMenu(uint Pos)
	{
		return apiDeleteMenu(m_SysMenu, Pos, 0);
	}
	
	// Retrieves a new object from a Window object
	public static SystemMenu FromWindow ( Window Win )
	{
		return FromWindow(new WindowInteropHelper(Win).Owner);
	}

	// Retrieves a new object from a window handle
	public static SystemMenu FromWindow ( IntPtr Handle )
	{
		SystemMenu cSysMenu = new SystemMenu();

		cSysMenu.m_SysMenu = apiGetSystemMenu(Handle, 0);
	...
	public static void ResetSystemMenu ( Window Win )
	{
		apiGetSystemMenu(new WindowInteropHelper(Win).Owner, 1);
	}
